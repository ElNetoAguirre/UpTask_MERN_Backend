...
const obtenerProyecto = async (req, res) => {
  ...
  // Comprobar que el creador sea el mismo que el usuario autenticado
  if(proyecto.creador.toString() !== req.usuario._id.toString() && !proyecto.colaboradores.some(colaborador => colaborador._id.toString() === req.usuario._id.toString())) {
    const error = new Error("Acción No Válida, se requiere Autenticación")
    return res.status(401).json({msg: error.message})
  }
  // Obtener las tareas del Proyecto
  // Para que no mande la información de las tareas en una misma cansulta y sea más limpio, se puede eliminar la petición
